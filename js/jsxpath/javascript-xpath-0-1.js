...
/*@cc_on
        ...
        return nodeset;
@*/
        if (attrName == 'id' && node.getElementById) {
            node = node.getElementById(attrValue);
            if (node && test.match(node)) {
                nodeset.push(node);
            }
        }
        else if (attrName == 'name' && node.getElementsByName) {
        ...
        else if (attrName == 'class' && node.getElementsByClassName) {
...
/*@end @*/
    ...
    translate: [function(s1, s2, s3) {
        ...
        for (var t = '', i = 0, l = s1.length; i < l; i ++) {
            var ch = s1.charAt(i);
            var replace = map[ch]
            t += typeof replace != 'undefined' ? replace : ch;
        }
        return t;
    }, 'string', false, []],
...
var NodeID = {
    uuid: 1,
    get: function(node) {
        return node.__xpath_id__ || (node.__xpath_id__ = this.uuid++);
    }
};
/*@end @*/
...
NodeSet.prototype.delDescendant = function(elm, offset) {
    ...
    if (uai.applewebkit && uai.applewebkit.match(/^4/)) return
...
/*@cc_on @if (@_jscript)
    ...
    var first, firstSourceIndex, firstSubIndex, sourceIndex, subIndex;
...
window.XPathExpression = function(expr) {
    if (!expr.length) {
...
XPathExpression.prototype.evaluate = function(node, type) {
    return new XPathResult(this.expr.evaluate(new Ctx(node)), type);
};

window.XPathResult = function (value, type) {
    if (type == 0) {
        switch (typeof value) {
            case 'object':  type ++; // 4
            case 'boolean': type ++; // 3
...
XPathResult.prototype.snapshotItem = function(i) { return this.nodes[i] };
...
document.createExpression = function(expr) {
    return new XPathExpression(expr, null);
};

document.evaluate = function(expr, context, _, type) {
