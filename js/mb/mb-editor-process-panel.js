...
var ProcessPanel = function(el, config) {
  this.processDef = new mb.ProcessDef();
  this.process = new mb.ProcessInstance(this.processDef, config.parentProc);
...
Ext.extend(ProcessPanel, Ext.NestedLayoutPanel, {
  loadConfig : function(processConfig) {
    ...
    this.process = new mb.ProcessInstance(this.processDef);
  ...
  setProcessName : function(name) {
    ...
    if (name && name.length > 0) this.setTitle(mb.string.escapeHTML(name));
  }
  ,
  destroy : function() {
    this.process.destroy();
