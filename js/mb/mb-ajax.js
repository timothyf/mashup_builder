mb.ajax = {
  ...
  request : function(params, callback) {
    ...
    params = mb.lang.extend({
      timeout : 10000
    }, params);

    mb.debug.log(params.method + ' ' + params.path);

    var req = this.createXMLHttpRequest();
    req.onreadystatechange = function() {
      if (req.readyState==4) {
        if (req.status>=200 && req.status<=206) {
          var result = mb.ajax.parseResponse(params.mimeType, req);
  ...
  parseResponse : function(mimeType, req) {
    if (mimeType=='text/json' || mimeType=='application/json') {
      return mb.lang.parseJSON(req.responseText); 
  ...
  jsonp : {
    ...
    invoke : function (url, callback, options) {
      ...
      var callbackContainer = options.globalCallback ? window : mb.ajax.jsonp.callbacks;
      var callbackPrefix = options.globalCallback ? '' : 'mb.ajax.jsonp.callbacks.';
      var callbackName = options.globalCallback ? 'mb_ajax_jsonp_callbacks'+index : index; 
      script.src = url + (url.indexOf('?')>0 ? '&' : '?') + 
                   options.jsonpParam + '=' + callbackPrefix + callbackName;

      mb.debug.log(script.src);
      ...
      window.setTimeout(function() {
        if (callbackContainer[callbackName]) {
          callbackContainer[callbackName] = mb.lang.emptyFunc; // suppress error
          script.parentNode.removeChild(script);
