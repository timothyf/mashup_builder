...
var plotr = new mb.UnitActionPackage('Renderer.Plotr', {
  label : 'Plotr Renderer'
});
 
plotr.register(new mb.RenderingUnitAction({
...
function renderPieChart(doc, records, width, height, labelField, valueField, colorScheme) {
  ...
  mb.lang.forEach(records, function(r, i) {
    ...
    var seq = mb.lang.map(
      mb.lang.cast('float[]', r[valueField] || r),
...
'        src="'+mb.packages.scriptBaseURL+'/renderers/plotr/lib/prototype/prototype.js">',
'</script>',
'<script type="text/javascript" ',
'        src="'+mb.packages.scriptBaseURL+'/renderers/plotr/lib/excanvas/excanvas.js">',
'</script>',
'<script type="text/javascript" ',
'        src="'+mb.packages.scriptBaseURL+'/renderers/plotr/plotr_uncompressed.js">',
'</script>',
'<script type="text/javascript">',
...
'  render('+mb.lang.toJSON(dataset)+','+mb.lang.toJSON(options)+');',
'}',
'</script>',
'</head>',
...
function renderBarChart(doc, records, width, height, labelField, valueFields, colorScheme) {
  ...
  var x = {};
  x.ticks = mb.lang.map(records, function(record, i) {
  ...
  var dataset = {};
  mb.lang.forEach(valueFields, function(valueField) {
    dataset[valueField] = mb.lang.map(records, function(record, i) {
      return [ i, mb.lang.cast('float', record[valueField]) ]; 
    });
  });
...
mb.packages.register(plotr, 'plotr-mb.js');
