...
        { text : 'Publish as HTML',
 ...
    });  
  ...
  loadProcessConfig : function(processConfig) {
    ...
    var scripts = mb.lang.values(requires);
    this.loadLibraries(scripts, function() {
      this.startLoading();
      var namespaces = mb.lang.keys(requires);
      mb.packages.waitLoadComplete(
        namespaces,
        function() {
  ...
  endLoading : function() {
    ...
    mb.lang.delay(Ext.Msg.hide, 100).call(Ext.Msg);
  }
  ,
  loadWADL : function(wadlUrl, callback){
    this.loadLibraries([ mb.config.WADL_CRE_SERVICE_URL+'?wadl='+wadlUrl ], callback);
  }
  ,
  ...
  loadLibraries : function(scripts, callback) {
    ...
    function _loadLibraries() {
      mb.lang.forEach(scripts, function(script) { 
        mb.packages.loadScript(script); 
      });
  ...
  loadProcessFromServer : function(key) {
    ...
    mb.processes.loadProcessConfig(key, this.loadProcessConfig.createDelegate(this));
  }
  ,
  ...
  handleProcessConfig : function(btn, txt) {
    if (btn=='ok') {
      var processConfig = mb.lang.parseJSON(txt);
      this.newProcess();
  ...
  saveProcessToLocal : function() {
    ...
    Ext.MessageBox.show({
      ...
      closable : true,
      value : mb.lang.toJSON(processConfig),
      multiline : true
   });
  }
  ,
  ...
  generateJavaScript: function(onload, widget) {
    ...
    mb.lang.forEach(mb.lang.values(procConfig.requires), function(scriptURL) {
	...
	buf.push('<script type="text/javascript" src="'+mb.packages.scriptBaseURL+'/mb-config.js"></script>');
	buf.push('<script type="text/javascript" src="'+mb.packages.scriptBaseURL+'/mb-core.js"></script>');
    buf.push('<script type="text/javascript" src="'+mb.packages.scriptBaseURL+'/mb-el.js"></script>');
	buf.push('<script type="text/javascript" src="'+mb.packages.scriptBaseURL+'/mb-ajax.js"></script>');
	buf.push('<script type="text/javascript" src="'+mb.packages.scriptBaseURL+'/mb-process.js"></script>');
	buf.push('<script type="text/javascript" src="'+mb.packages.scriptBaseURL+'/mb-action.js"></script>');
	buf.push('<script type="text/javascript" src="'+mb.packages.scriptBaseURL+'/mb-render.js"></script>');
    ...
    mb.lang.forEach(procConfig.params, function(param, i) {
      if(widget)
  ...
  showTestDialog : function() { 
    ...
    var msgWindow = window.open(mb.config.EMPTY_URL,"","width=600,height=450,scrollbars=yes,centerscreen=yes,menubar=no,location=no");
