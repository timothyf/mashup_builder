...
var ProcessDef = mb.lang.defineClass({
  // name : String
  ...
  loadConfig : function(config) {
    ...
    mb.lang.forEach(config.params || [], function(pconfig) {
      _this.addParamDef(pconfig);
    });
    this.output = config.output;

    mb.lang.forEach(config.actions || [], function(aconfig) {
  ...
  toConfig : function() {
    return {
      ...
      requires : mb.packages.getDependencies(this),
      params : mb.lang.values(this.params),
      output : this.output || '',
      actions : jQuery.map(
                  mb.lang.values(this.actions).sort(function(a1, a2) {
  ...
  ,
  serialize : function() {
    return mb.lang.toJSON(this.toConfig());
  }
  ,
  destroy : function() {
    var _this = this;
    mb.lang.forEach(mb.lang.values(this.actions), function(actionDef) {
...
var ActionDef = mb.lang.defineClass({
  // name : String
  // uaction : UnitAction
  ...
  loadConfig : function(config) {
    ...
    this.uaction = mb.packages.findUnitAction(config.type);
...
var ProcessRegistry = mb.lang.defineClass({
  ...
  loadProcessConfig : function(key, callback) {
    ...
    mb.lang.loadScript(mb.config.PROCESS_REGISTRY_URL + '?id=' + key);
  }
  ,
...
mb.processes = new ProcessRegistry();


var ProcessInstance = mb.lang.defineClass({
  ...
  setParam : function(name, value) {
    ...
    if (paramDef) {
      value = mb.lang.cast(paramDef.type, value);
    }
  ...
  resolveRef : function(name) {
    ...
    if (!ref) {
      ...
      if (paramDef && typeof paramDef['default'] != 'undefined') {
        var value = mb.lang.cast(paramDef.type, paramDef['default']); 
        this.params[name] = new ProcessParam(value);
    ...
    mb.async.map(
      ...
      { 
        onSuccess : function(results) {
          var timestamp = 0;
          var context = {};
          mb.lang.forEach(results, function(result) {
  ...
  ,
  destroy : function() {
    mb.lang.forEach(mb.lang.values(this.actions), function(action) {
      action.destroy();
    })
  }
...
var ProcessParam = mb.lang.defineClass({
  // value : Object
  // timestamp : integer
  ...
  evaluate : function(callback) {
    mb.lang.delay(callback.onSuccess, 1)
               .call(callback, { value : this.value, timestamp : this.timestamp })
  }
})

mb.ProcessDef = ProcessDef;
mb.ActionDef = ActionDef;
mb.ProcessRegistry = ProcessRegistry;
mb.ProcessParam = ProcessParam;
mb.ProcessInstance = ProcessInstance;
