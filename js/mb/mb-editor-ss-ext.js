...
Ext.extend(EditorAppExt, mb.editor.MashupBuilder, {
  ...
  saveProcessToLocal : function() {
    ...
    this.iframeSaveFile.src = mb.config.PROCESS_SAVE_LOC_ULR+'?json='+mb.url.urlEncode(mb.lang.toJSON(processConfig));
  }
  ,
  ...
  saveProcessToServer : function(copying) {
    ...
    {
      var processConfig = this.rootProcessPanel.processDef.toConfig();
      var json = mb.lang.toJSON(processConfig);

      var conn = new Ext.data.Connection();
      conn.request({
        url: mb.config.PROCESS_SAVE_URL,
        method: 'POST',
...
Ext.extend(OpenProcessDialog, mb.editor.AbstractCallbackDialog, {

  setup : function() {

    this.ds = new Ext.data.Store({
      proxy: new Ext.data.ScriptTagProxy({
        url: mb.config.PROCESS_PRIVATE_URL
      }),
...
var OpenProcessLocalDialog = function(editor) {
  ...
  '<form id="fileForm" method="POST" enctype="multipart/form-data" action="'+mb.config.PROCESS_OPEN_URL+'" target="contentFileIframe">',
  '  <input type="file" name="fileInput" id="fileInput" value="" size="38" onchange="mb.editor.FileChooserEvent.onChangeFileChooser();" />',
  '</form>',
...
Ext.extend(OpenProcessLocalDialog, mb.editor.AbstractCallbackDialog, {

  submit : function() {
    var processConfig = mb.lang.parseJSON(document.getElementById("textareaFileContent").value);
  ...
  show: function() {
    this.PID = window.setInterval(function(){
      if(mb.editor.OpenProcessLocalDialog.submit > 0)
      {
        ...
        if(document.getElementById("textareaFileContent").value != text)
        {
          ...
          mb.editor.OpenProcessLocalDialog.submit--;
        }
      }
    }, 100);
...
Ext.extend(SaveProcessDialog, mb.editor.AbstractCallbackDialog, {
...
var FileChooserEvent = mb.lang.defineClass({
...
mb.editor.MashupBuilder = EditorAppExt;

mb.editor.FileChooserEvent = new FileChooserEvent();
mb.editor.OpenProcessLocalDialog = {};
mb.editor.OpenProcessLocalDialog.submit = 0;
