...
array.register(new afrous.UnitAction({
  ...
  execute : function(request, callback) {
    ...
    if (action.definition.innerProcess && action.definition.innerProcess.output) {
      mbuilder.async.iterate(
        arr, 
        function(a, i, arr, cb) {
          var childProc = new mbuilder.ProcessInstance(action.definition.innerProcess, process);
    ...
    function checkCondition(element, condition) {
      switch (condition.operand) {
        ...
        case '!=' : 
          return element[condition.property] != condition.value;
        case 'includes' : 
          var pstr = mbuilder.lang.toString(element[condition.property]);
...
array.register(new afrous.UnitAction({
  ...
  execute : function(request, callback) {
    ...
    if (arr) {
      mbuilder.lang.forEach(arr, function(elem) {
        if (elem.constructor == Array) {
          mbuilder.lang.forEach(elem, function(a) { rarr.push(a) });
        } else {
          rarr.push(elem);
        }
      })
...
array.register(new afrous.UnitAction({
  ...
  execute : function(request, callback) {
    ...
    var rarr = jQuery.grep(request.params['array'] || [], function(elem) {
      var val = property ? elem[property] : mbuilder.lang.toJSON(elem);
      if (tmp[val]) {
        return false;
      } else {
        tmp[val] = true;
