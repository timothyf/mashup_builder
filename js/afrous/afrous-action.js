 ...
 * UnitAction
 */
var UnitAction = mbuilder.lang.defineClass({
  // type : String
  // label : String
  // description : String
  ...
  initialize : function(props) {
    mbuilder.lang.extend(this, props);
    this.inputs = {};
    var _this = this;
    mbuilder.lang.forEach(props.inputs || [], function(inputDef) {
...
var UnitActionPackage = mbuilder.lang.defineClass({
  ...
  listUnitActions : function() {
    return mbuilder.lang.values(this.uactions);
  }
  ,
...
var UnitActionPackageRegistry = mbuilder.lang.defineClass({ 
  ...
  register : function(pack, scriptname) {
    ...
    if (scriptname) {
      ...
      var scriptURL = mbuilder.lang.find(
        this.scripts, 
  ...
  loadScript : function(scriptUrl, overwrite) {
    var s = mbuilder.lang.find(this.scripts, function(s){ return s==scriptUrl });
    if (s && !overwrite) return;
    if (!s) this.scripts.push(scriptUrl);
    mbuilder.lang.loadScript(scriptUrl);
  }
  ,
  ...
  getDependencies : function(processDef) {
    ...
    function _getDependentScripts(procdef) {
      mbuilder.lang.forEach(mbuilder.lang.values(procdef.actions), function(actionDef) {
  ...
  listPackages : function() {
    return mbuilder.lang.values(this.packages);
  }
  ,
  toConfig : function() {
    var _this = this;
    return jQuery.map(
      mbuilder.lang.keys(this.nsmap),
      function(namespace) {
...
var ActionInstance = mbuilder.lang.defineClass({
  ...
  evaluate : function(callback) {
    ...
    if (this.status == ActionInstance.Status.WAIT) {
      mbuilder.lang.delay(this.prepare, 1)
  ...
  dispatch : function(request) {
    ...
    while(this.callbackQueue.length>0) {
      try {
        ...
        mbuilder.lang.delay(callback.onSuccess, this.definition.uaction.waittime || 1)
                   .call(callback, { value : this.value, timestamp : this.timestamp });
      } catch(e) { 
        mbuilder.debug.log(e)
      }
    }
  ...
  error : function(request, error) {
    ...
    while(this.callbackQueue.length>0) {
      var callback = this.callbackQueue.pop();
      mbuilder.lang.delay(callback.onFailure, this.definition.uaction.waittime || 1)
                 .call(callback, error)
    }
    this.changeStatus(ActionInstance.Status.WAIT);
  }
  ...
  statusDebug : function(status) {
    mbuilder.debug.log(this.definition.name + ': ' + this.status);
  }
  ,
  destroy : function() {
...
var ActionRequest = mbuilder.lang.defineClass({
      ...
      onSuccess : function(result) {
        ...
        for (var name in result.value) {
          ...
          _this.params[name] = inputDef ? 
                              mbuilder.lang.cast(inputDef.type, value) :
                              value;
        }
        next.onSuccess();
      },
