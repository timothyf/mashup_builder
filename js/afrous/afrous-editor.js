...
Ext.extend(ParamsTreeNode, ObjectTreeNode, {
  setValue : function(value) {
    if (this.type) {
      value = mbuilder.lang.cast(this.type, value);
    }
...
Ext.extend(ActionsPanel, Ext.NestedLayoutPanel, {
  loadConfig : function(actionCfgs) {
    mbuilder.lang.forEach(actionCfgs, this.createActionWidget.createDelegate(this));
  }
  ,
  repaintWidgets : function() {
    mbuilder.lang.forEach(this.widgets, function(w) { w.repaint() });
  }
  ,
  handleOver : function(source, event, data) {
    return "x-dd-drop-ok";
  ...
  createActionWidget : function(config) {
    var actionDef = new mbuilder.ActionDef(config);
    try {
  ...
  insertWidgetAfter : function(pivot, widget) {
    ...
    mbuilder.lang.forEach(this.widgets, function(w) {
      if (w==widget) {
        return;
      }
      tmpwidgets.push(w);
  ...
  handleFocusWidget : function(widget) {
    mbuilder.lang.forEach(this.widgets, function(wgt) {
  ...
  recalcActionDefOrder : function() {
    mbuilder.lang.forEach(this.widgets, function(widget, i) {
  ...
  onLayout : function(event) {
    ...
    this.layout.layout();
    //mbuilder.lang.forEach(this.widgets, function(w) { w.adjustSize(); });
  }
  ,
  ...
  destroy : function() {
    ...
    mbuilder.lang.forEach(this.widgets, function(w) { w.destroy() });
  }
});
  ...
  renderInputFields : function() {
    ...
    if (this.uaction.inputs) {
      mbuilder.lang.forEach(
        mbuilder.lang.values(this.uaction.inputs), 
        function(inputDef) { this.createBindField(inputDef) },
        this
      ); 
    }
    mbuilder.lang.forEach(
      mbuilder.lang.keys(this.actionDef.inputs || {}),
      function(key) {
        var field = mbuilder.lang.find(this.inputFields, function(f){ 
...
Ext.extend(ProcessActionWidget, ActionWidget, {
  ...
  _openInnerProcessEditor : function() { 
    ...
    action.prepareInnerParams(function(params) {
      mbuilder.lang.forEach(mbuilder.lang.keys(params||{}), function(pname) {
 ...
 * @class WidgetManager
 */
var WidgetManager = mbuilder.lang.defineClass({
  initialize : function() {
    this.widgets = {};
  }
  ,
...
afrous.editor.MashupBuilder = MashupBuilder;
